...
func Render(pkg string, licenses []spdx.License) string {
	f := jen.NewFilePath(pkg)

	//https://github.com/golang/go/issues/13560#issuecomment-277804473
	f.PackageComment("Code generated by github.com/joshdk/license/spdxgen. DO NOT EDIT.")

	values := make([]jen.Code, len(licenses))

	for index, license := range licenses {
		values[index] = license.JenValue()
	}

	f.Func().Id("init").Params().Block(
		jen.Id("licenses").Op("=").Index().Qual("github.com/joshdk/license/spdx", "License").Values(values...),
	)
